...
def main():
    ...
    prompt = f"""
Ты опытный Python-разработчик и code reviewer.

Анализируй только содержимое файла ниже.

Очень важно:
- Не придумывай проблемы, которых нет.
- Не советуй логирование, тесты, безопасность и обработку ошибок просто "на всякий случай".
- Указывай только реальные замечания, которые видны в коде.
- Если серьёзных проблем нет, так и напиши.
- Не пиши общие советы.
- Для каждого замечания показывай конкретный фрагмент кода или функцию.
